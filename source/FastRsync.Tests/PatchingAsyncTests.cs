...
namespace FastRsync.Tests
{
    ...
    public class PatchingAsyncTests
    {
        ...
        {
            // Arrange
            var (baseDataStream, baseSignatureStream, newData, newDataStream) = await PrepareTestDataAsync(baseNumberOfBytes, newDataNumberOfBytes, chunkSize, SupportedAlgorithms.Checksum.Adler32Rolling()).ConfigureAwait(false);
        ...
        [TestCase(6666, 6666, SignatureBuilder.DefaultChunkSize)]
        [TestCase(6666, 6666, SignatureBuilder.MaximumChunkSize)]
        public async Task PatchingAsyncXXHashAdlerV2_ForNewData_PatchesFile(int baseNumberOfBytes, int newDataNumberOfBytes, short chunkSize)
        {
            // Arrange
            var (baseDataStream, baseSignatureStream, newData, newDataStream) = await PrepareTestDataAsync(baseNumberOfBytes, newDataNumberOfBytes, chunkSize, SupportedAlgorithms.Checksum.Adler32RollingV2()).ConfigureAwait(false);
